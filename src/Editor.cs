...
public class Editor{
	...
	public static bool loadFromPath(string password = null){
		try{
			...
			if(e.Message == "File is password protected, and no password was provided"){
				...
				return loadFromPath(pw);
			}else if(e.Message == "Error occured on decryption"){
				writeLine("Tip: password might be incorrect", paleRed);
				string pw = askPassword();
				if(pw == null){
					return false;
				}
	...
	public static void info(){
		writeLine("The current version of AshFile Editor is v4.0.1", purple);
